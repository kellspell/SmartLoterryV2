    ...
    : describe("Raffle Unit Test", function () {
          ...
          describe("Constructor", function () {
              it("initializes the raffle correctly", async function () {
                  // Ideally we want our test to have just 1 assert per "it"
              ...
              describe("Enter Raffle", function () {
                  it("Reverts if you dont have enough ETH", async function () {
                      await expect(raffle.enterRaffle()).to.be.revertedWith(
                          "Raffle__SendMoreToEnterRaffle"
                      )
                  })
                  it("Records players when they enter to the Raffle", async function () {
                      await raffle.enterRaffle({ value: raffleEntranceFee })
                      const playerFromContract = await raffle.getPlayer(0)
                      assert.equal(playerFromContract, deployer)
                  })
                  it(" Check it if Emits events are trigged on the Enter ", async function () {
          ...
          describe("PerformUpkeep", function () {
              ...
              it("Updates the raffle state, Emits and event , and calls the VRF Coodinator", async function () {
                  await raffle.enterRaffle({ value: raffleEntranceFee })
                  await network.provider.send("evm_increaseTime", [interval.toNumber() + 1])
                  await network.provider.request({ method: "evm_mine", params: [] })
          ...
          describe("FulfillRandomWords", function () {
              ...
              it("Picks a winner, reset the lottery, and sends the money out ", async function () {
                  ...
                  await new Promise(async (resolve, reject) => {
                      raffle.once("WinnerPicked", async () => {
                          console.log("Found the event fired!")
                          try {
                      ...
                      // bellow , We'll fire the event, and the listener will pick it up , and resolve
                      const tx = await raffle.performUpkeep([])
